...
<form action="{{ route('admin.update_offer', $offer->id) }}" method="POST">
    ...
    <div class="card p-4">
        ...
        <div class="form-group">
            ...
            <textarea name="description" class="form-control" required>{{ old('description', $offer->description) }}</textarea>
        </div>
        ...
        <div class="form-group">
            ...
            <!-- All -->
            <div>
                <input type="radio" name="driver_type" value="all" 
                       {{ $offer->driver_type == 'all' ? 'checked' : '' }}> All Drivers
            </div>

            <!-- Company -->
            <div>
                <input type="radio" name="driver_type" value="company" 
                       {{ $offer->driver_type == 'company' ? 'checked' : '' }}> By Company
            </div>

            <div id="companyFields" style="display:{{ $offer->driver_type=='company' ? 'block' : 'none' }};">
                <input type="text" name="mc_number" class="form-control mt-2" 
                       placeholder="MC Number" value="{{ $offer->mc_number }}">
                <input type="text" name="dot_number" class="form-control mt-2" 
                       placeholder="DOT Number" value="{{ $offer->dot_number }}">
            </div>

            <!-- Exact -->
            <div>
                <input type="radio" name="driver_type" value="exact" 
                       {{ $offer->driver_type == 'exact' ? 'checked' : '' }}> Exact Driver
            </div>

            <div id="phoneField" style="display:{{ $offer->driver_type=='exact' ? 'block' : 'none' }};">
                ...
                <div id="selectedDrivers" class="mt-2">
                    @if($offer->phone_number)
                        @foreach(json_decode($offer->phone_numbers) as $phone)
                            <div style="display:inline-block; margin:5px; padding:6px 10px; background:#007bff; color:#fff; border-radius:5px;">
                                {{ $phone }}
                                <input type="hidden" name="phone_numbers[]" value="{{ $phone }}">
                                <span class="remove-driver" style="cursor:pointer; margin-left:8px;">×</span>
                            </div>
                        @endforeach
                    @endif
                </div>
